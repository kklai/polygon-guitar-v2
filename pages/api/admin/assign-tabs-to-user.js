...
export default async function handler(req, res) {
  ...
  try {
    ...
    const skipCacheRebuild = !!req.body?.skipCacheRebuild
    ...
    if (results.successCount > 0 && !skipCacheRebuild) {
      try {
    ...
    console.error('[assign-tabs-to-user]', e?.message)
    return res.status(500).json({
      error: e?.message || '移植失敗'
    })
  }
}
