...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = {
      ...
      // 作曲填詞等（從 work-rels 提取）
      credits: extractCredits(detailsData),
...
// 從 MusicBrainz 數據提取作曲填詞
function extractCredits(detailsData) {
  const credits = {
    composers: [],
    lyricists: [],
    arrangers: [],
  ...
  if (!detailsData?.relations) return credits
  ...
  // 去重
  credits.composers = [...new Set(credits.composers)]
  credits.lyricists = [...new Set(credits.lyricists)]
  credits.arrangers = [...new Set(credits.arrangers)]
