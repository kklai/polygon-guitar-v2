...
import { Music, Share2, Heart, BookmarkPlus } from 'lucide-react';
...
export default function UserPlaylistDetail() {
  ...
  const savePlaylistTitle = async () => {
    ...
    try {
      await updateDoc(doc(db, 'userPlaylists', id), {
        title: trimmed,
        updatedAt: serverTimestamp()
      });
      setPlaylist({ ...playlist, title: trimmed });
      setShowEditTitle(false);
    } catch (error) {
  ...
  const coverUrl = songs.length > 0 ? getSongThumbnail(songs[0]) : null;
  ...
  return (
    <Layout fullWidth hideHeader>
      ...
      <div className="relative z-10 min-h-screen pb-24 bg-black" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
        ...
        {/* 標題行：歌單名 + 編輯 icon + 共 X 首 • 由 xxx 創建 */}
          ...
          <div className="flex items-baseline justify-between gap-3">
              ...
              {showEditTitle ? (
                <>
                  <input
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && savePlaylistTitle()}
                    className="flex-1 min-w-0 bg-[#282828] text-white font-bold px-3 py-1.5 rounded-lg outline-none focus:ring-2 focus:ring-[#FFD700]"
                  ...
                  <button
                    ...
                    className="flex-shrink-0 px-3 py-1.5 bg-[#FFD700] text-black font-medium rounded-lg hover:opacity-90 disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="flex-shrink-0 p-1.5 text-[#B3B3B3] hover:text-white rounded-lg disabled:opacity-50"
                  >
                    取消
                  </button>
                </>
                ...
                <>
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg text-[#B3B3B3] hover:text-white hover:bg-white/10 transition flex-shrink-0"
                    title="更改歌單名稱"
                  >
            ...
            {!showEditTitle && (
              <span className="text-[12px] md:text-[14px] text-gray-500 whitespace-nowrap flex-shrink-0">
                共 {songs.length} 首
                {user?.displayName && <> • 由 {user.displayName} 創建</>}
              </span>
            )}
          </div>
        </div>

        {/* Action Bar：排序 icon（同歌單頁） */}
        {songs.length > 0 && (
