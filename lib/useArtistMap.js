...
export function resolveArtistName(tab, artistMap) {
  if (!tab) return ''
  if (tab.artist) return tab.artist
  if (tab.artistName) return tab.artistName
  if (artistMap && tab.artistId) {
    const name = artistMap.get(tab.artistId) || artistMap.get(tab.artistId.toLowerCase())
    return name || tab.artistId || ''
  }
  return tab.artistId || ''
}
