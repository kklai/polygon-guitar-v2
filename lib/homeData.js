...
/** Only fields needed for home SongCard: id, title, artistId; one image — coverImage, or artistPhoto only when no cover (saves payload). 多歌手時帶上 artist 顯示名（如 MC 張天賦 feat. Billy Choi）供首頁顯示；否則帶上主歌手名（tab.artist / tab.artistName 或 artistNameById[artistId]）確保首頁不依賴 search-data 都有對應 artist。artistId 用 case-insensitive 對應（Firestore 歌手 id 多為小寫）。 */
function slimTabForHome(tab, photoMap, artistNameById = null) {
  if (!tab) return tab
  const coverImage = resolveTabCoverImage(tab)
  const artistPhoto =
    !coverImage && photoMap && (photoMap[tab.artistId] || (tab.artistId && photoMap[tab.artistId.toLowerCase()]))
  const artistDisplay = Array.isArray(tab.collaborators) && tab.collaborators.length > 0
    ...
    : (tab.artist || tab.artistName || resolveArtistNameFromMap(artistNameById, tab.artistId))
  return {
    id: tab.id,
    title: tab.title,
