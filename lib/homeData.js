...
/** Only fields needed for home SongCard: id, title, artistId; one image — coverImage, or artistPhoto only when no cover (saves payload). Supports legacy (tab.artistId) and new (tab.artists / tab.artistIds) shape. */
function slimTabForHome(tab, photoMap, artistNameById = null) {
  ...
  const artistDisplay = Array.isArray(tab.collaborators) && tab.collaborators.length > 0
    ? (tab.collaborationType === 'feat' ? tab.collaborators.join(' feat. ') : tab.collaborators.join(' / '))
    : artistIds.length > 1
      ? artistIds.map(id => resolveArtistNameFromMap(artistNameById, id) || id).join(tab.collaborationType === 'feat' ? ' feat. ' : ' / ')
      : (resolveArtistNameFromMap(artistNameById, mainArtistId) || tab.artist || tab.artistName)
  return {
    id: tab.id,
    title: tab.title,
