...
// Get tabs by artist — queries by Firestore doc ID (single source of truth).
export async function getTabsByArtist(artistName, artistDocId = null) {
  ...
  const [byArtistIdSnap, byCollaboratorSnap] = await Promise.all([
    getDocs(query(collection(db, TABS_COLLECTION), where('artistId', '==', artistId))).catch(() => null),
    getDocs(query(collection(db, TABS_COLLECTION), where('collaboratorIds', 'array-contains', artistId))).catch(() => null)
  ])
  ...
  console.log(`[getTabsByArtist] ${artistName} (${artistId}) — ${allTabs.length} tabs in ${Date.now() - startMs}ms (byId: ${byArtistIdSnap?.docs?.length ?? 0}, byCollab: ${byCollaboratorSnap?.docs?.length ?? 0}) at ${pacificTime()}`)
