...
// 生成歌手的 normalized ID（保留 . 用於處理如 Gareth.T）
...
/** Slim tab for cache: all fields except content to stay under 1 MiB. */
...
async function getAllTabsFromFirestoreCache() {
  try {
    ...
    if (!data || !Array.isArray(data) || !updatedAt || typeof updatedAt.toMillis !== 'function') return null
...
// Firestore 'in' and 'array-contains-any' allow max 10 values
const MAX_ARTIST_ID_VARIANTS = 10

/**
 * Build up to 10 artist id variants for matching (alternate / ascii / chinese).
 * Use with artistId 'in' and collaboratorIds 'array-contains-any' so we only need 2 queries.
 */
function getArtistIdVariants(artistName, artistIdParam) {
  const artistIdFromName = artistName.toLowerCase().replace(/\s+/g, '-')
  const artistId = artistIdParam || artistIdFromName
  const asciiOnlyId = artistName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9\.\-]/g, '').replace(/-+$/, '')
  const chineseMatch = artistName.match(/^([\u4e00-\u9fa5]{2,})/)
  const chineseId = chineseMatch && chineseMatch[1] !== artistName
    ? chineseMatch[1].toLowerCase().replace(/\s+/g, '-')
    : null
  const ids = [artistId, artistIdFromName, asciiOnlyId, chineseId].filter(Boolean)
  const seen = new Set()
  const out = []
  for (const id of ids) {
    if (!seen.has(id) && out.length < MAX_ARTIST_ID_VARIANTS) {
      seen.add(id)
      out.push(id)
    }
  }
...
// Get tabs by artist — 2 queries only: artistId in variants, collaboratorIds array-contains-any variants.
// Legacy tabs with only "artist" (no artistId) won't match; backfill artistId from artist to include them.
export async function getTabsByArtist(artistName, artistIdParam = null) {
  const artistId = artistIdParam || artistName.toLowerCase().replace(/\s+/g, '-')
  const cacheKey = `artistTabs_${artistId}_${artistName}`
  const cached = getCached(cacheKey)
  if (cached) return cached

  const idVariants = getArtistIdVariants(artistName, artistIdParam)
  ...
  const startMs = Date.now()
  const [byArtistIdSnap, byCollaboratorSnap] = await Promise.all([
    getDocs(query(collection(db, TABS_COLLECTION), where('artistId', 'in', idVariants))).catch(() => null),
    getDocs(query(collection(db, TABS_COLLECTION), where('collaboratorIds', 'array-contains-any', idVariants))).catch(() => null)
  ])

  const seenIds = new Set()
  const allTabs = []
  for (const snapshot of [byArtistIdSnap, byCollaboratorSnap]) {
  ...
  console.log(`[getTabsByArtist] ${artistName} — ${allTabs.length} tabs in ${Date.now() - startMs}ms (byId: ${byArtistIdSnap?.docs?.length ?? 0}, byCollab: ${byCollaboratorSnap?.docs?.length ?? 0}) at ${pacificTime()}`)
...
export async function updateTab(id, tabData, userId, isAdmin = false) {
  ...
  // 清理數據，移除 undefined 值（含巢狀物件/陣列，Firestore 不接受 undefined）
