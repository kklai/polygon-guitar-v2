...
export async function createTab(tabData, userId) {
  // Create or get artist with photo/bio/year/type
  const artistData = {
  ...
  // Increment artist tab count (only if artist document exists)
  if (artistId) {
    const artistRef = doc(db, ARTISTS_COLLECTION, artistId)
    const artistSnap = await getDoc(artistRef)
    if (artistSnap.exists()) {
...
export async function updateTab(id, tabData, userId, isAdmin = false) {
  ...
  const isOldTab = !currentTab.createdBy  // 舊譜沒有 createdBy
  ...
  if (oldArtistId !== newArtistId && tabData.artist) {
    // Decrement old artist count (only if artist document exists)
    if (oldArtistId) {
    ...
    // Create/get new artist with photo/bio/year/type and increment count
    const artistData = {
      photo: tabData.artistPhoto || null,
      bio: tabData.artistBio || '',
      year: tabData.artistYear || '',
  ...
  } else if (oldArtistId === newArtistId && (tabData.artistPhoto || tabData.artistBio || tabData.artistYear || tabData.artistType)) {
    // 同一歌手但更新了資料
    const artistData = {
      photo: tabData.artistPhoto || null,
      bio: tabData.artistBio || '',
...
export async function deleteTab(id, userId, isAdmin = false) {
  ...
  const isOldTab = !tab.createdBy  // 舊譜沒有 createdBy
  ...
  // Decrement artist tab count (only if artist document exists)
  if (tab.artistId) {
