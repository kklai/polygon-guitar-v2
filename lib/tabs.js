...
export async function getTabsByArtist(artistName, artistIdParam = null) {
  ...
  // 方法1b: 用 collaboratorIds 數組查詢（多歌手支持）
  try {
    const q1b = query(
      collection(db, TABS_COLLECTION), 
...
export async function createTab(tabData, userId) {
  ...
  const artistId = await getOrCreateArtist(primaryArtist || tabData.artist, artistData)
  
  // 為所有合作歌手創建記錄
  const collaboratorIds = collaborators.map(name => normalizeArtistId(name)).filter(Boolean)
