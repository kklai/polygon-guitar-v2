...
export async function getTabsByArtist(artistName, artistIdParam = null) {
  ...
  // 方法1: 用歌手ID查詢
  queries.push(getDocs(query(collection(db, TABS_COLLECTION), where('artistId', '==', artistId))).catch(() => null))

  // 方法1b: 用 collaboratorIds 查詢
  queries.push(getDocs(query(collection(db, TABS_COLLECTION), where('collaboratorIds', 'array-contains', artistId))).catch(() => null))

  // 方法1c: 用 collaboratorIds - ASCII only ID
  if (asciiOnlyId && asciiOnlyId !== artistId) {
    queries.push(getDocs(query(collection(db, TABS_COLLECTION), where('collaboratorIds', 'array-contains', asciiOnlyId))).catch(() => null))
  }

  // 方法2: 用 artistIdFromName 查詢
  if (artistIdFromName !== artistId) {
    queries.push(getDocs(query(collection(db, TABS_COLLECTION), where('artistId', '==', artistIdFromName))).catch(() => null))
  }

  // 方法3: 用 artist 名稱查詢（舊數據兼容）
  queries.push(getDocs(query(collection(db, TABS_COLLECTION), where('artist', '==', artistName))).catch(() => null))

  // 方法4: 雙語歌手名，用中文名查詢
  const chineseMatch = artistName.match(/^([\u4e00-\u9fa5]{2,})/)
  if (chineseMatch && chineseMatch[1] !== artistName) {
    const chineseId = chineseMatch[1].toLowerCase().replace(/\s+/g, '-')
    queries.push(getDocs(query(collection(db, TABS_COLLECTION), where('artistId', '==', chineseId))).catch(() => null))
  }
