...
/** Slim tab for artist page list: no content. Normalize artistId/artistIds for consumers. */
export function slimTabForArtistPage(t) {
  ...
  return {
    ...
    collaborationType: t.collaborationType,
...
export async function createTab(tabData, userId) {
  ...
  // Store only artists + artistIds (no artistId / collaborators / collaboratorIds)
  const artists = collaboratorIds.map((id, i) => ({ id, role: i === 0 ? 'main' : 'feat' }))
  ...
  const newTab = {
    ...
    artistIds,
    collaborationType,
    isCollaboration: collaboratorIds.length > 1,
    createdBy: userId,
    createdAt: new Date(),
    updatedAt: new Date(),
    likes: 0,
...
export async function updateTab(id, tabData, userId, isAdmin = false) {
  ...
  const { primaryArtist, collaborators: parsedNames, collaborationType } = parseCollaborators(tabData.artist || '')
  const artistData = {
    photo: tabData.artistPhoto || null,
    bio: tabData.artistBio || '',
    year: tabData.artistYear || '',
  ...
  if (Array.isArray(tabData.artists) && tabData.artists.length > 0) {
    ...
    for (const a of tabData.artists) {
      if (a?.id) {
        ...
        const docId = await getOrCreateArtist(a.name, artistData)
  ...
  const artists = newCollaboratorIds.map((id, i) => ({ id, role: i === 0 ? 'main' : 'feat' }))
  ...
  const updatedData = stripUndefined({
    ...
    artistIds,
    collaborationType,
    isCollaboration: newCollaboratorIds.length > 1,
...
export async function getArtistByIdOrSlug(id) {
  ...
  try {
    ...
    if (decoded !== id) lookupId = decoded
  } catch (_) { /* keep lookupId as id */ }
  ...
  const bySlug = await getArtistBySlug(lookupId)
