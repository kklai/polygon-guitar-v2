 ...
 * 1. FIREBASE_SERVICE_ACCOUNT = path to service account JSON (e.g. ./path/to/key.json)
 * 2. FIREBASE_ADMIN_PROJECT_ID + FIREBASE_ADMIN_CLIENT_EMAIL + FIREBASE_ADMIN_PRIVATE_KEY
 *    (or NEXT_PUBLIC_ variants for project ID / client email only; private key must be server-only)
 */
  ...
  try {
    if (getApps().length === 0) {
      if (process.env.FIREBASE_SERVICE_ACCOUNT) {
        if (!initFromServiceAccountFile()) return null
      } else if (!initFromEnvVars()) {
        return null
      }
    }
