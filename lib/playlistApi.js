  ...
  try {
    ...
    const list = await Promise.all(
      snapshot.docs.map(async (d) => {
        ...
        const savedAtMs = savedAt?.toMillis?.() ?? 0;
        if (!playlistSnap.exists()) return null;
        return { id: playlistSnap.id, ...playlistSnap.data(), savedAtMs };
      })
    );
  ...
  try {
    ...
    const list = await Promise.all(
      snapshot.docs.map(async (d) => {
        ...
        return { id: artistSnap.id, ...artistSnap.data(), savedAtMs };
      })
    );
    return list.filter(Boolean);
  } catch (error) {
...
export async function getPlaylistCovers(songIds) {
  ...
  try {
    for (let i = 0; i < Math.min(4, songIds.length); i++) {
      const tabRef = doc(db, 'tabs', songIds[i]);
      const tabDoc = await getDoc(tabRef);
      
      if (tabDoc.exists() && tabDoc.data().thumbnail) {
        covers.push(tabDoc.data().thumbnail);
      }
    }
    return covers;
  } catch (error) {
