...
function isChordLine(line) {
  ...
  // 和弦 pattern
  const chordPattern = /^[A-G](?:#|b)?(?:maj|mj|m|M|min|dim|aug|sus|add)?[0-9]*(?:\/[A-G](?:#|b)?)?$/;
  const ncPattern = /^N\.?C\.?$/i;
  
  // 大部分 tokens 係和弦就算係和弦行（NC = No Chord 當有效 token）
  const chordCount = tokens.filter(t => chordPattern.test(t) || ncPattern.test(t)).length;
 ...
 * @param {boolean} compressSpaces - 是否壓縮空格（Arial 模式設為 false）
 */
export function autoFixTabFormatWithFactor(content, factor = 1.1, compressSpaces = true) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isChordLine(line)) {
      // 根據設定決定是否壓縮空格
      const processedChordLine = compressSpaces 
        ? compressChordLineSpaces(line, factor)
        : line;
      result.push(processedChordLine);
      ...
      if (nextLine && 
        ...
        // 根據設定決定是否對齊歌詞
        const processedLyric = compressSpaces
          ? alignLyricToChord(processedChordLine, nextLine, factor)
          : nextLine;
        result.push(processedLyric);
        i++; // 跳過下一行
      }
    } else {
      // 普通行，保留原樣
