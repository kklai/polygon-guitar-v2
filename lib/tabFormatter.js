 ...
 * 3. 計算需要幾多個等寬空格
 */
function alignLyricToChord(chordLine, lyricLine) {
  ...
  const conversionFactor = 1.1; // 經驗值（調整到較大值以補償差異）
  const targetMonoCols = Math.round(targetArialWidth * conversionFactor);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isChordLine(line)) {
      ...
      if (nextLine && 
        ...
        // 對齊歌詞
        const alignedLyric = alignLyricToChord(line, nextLine);
