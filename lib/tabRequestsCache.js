 ...
 *
 * Reads use Client SDK (same as home-data, search-data) so they go through Firestore Rules and
 * don't use Admin quota. Writes use Admin SDK (cache has allow write: if false for client).
 */

import { doc, getDoc, collection, query, orderBy, getDocs } from '@/lib/firestore-tracked'
...
/**
 * Build full list from Firestore (Client SDK). Used on cold cache. N reads.
 * tabRequests has allow read: if true, so client SDK works from server.
 */
 ...
 * Uses Client SDK (cache has allow read: if true).
 */
export async function getTabRequestsCache() {
  try {
    const ref = doc(db, 'cache', CACHE_DOC_ID)
...
export async function mergeTabRequestsCache(payload) {
  ...
  const { action, id, doc: docData, voteCount, voters, songTitle, artistName, status, fulfilledBy, fulfilledByName, fulfilledAt, tabId, albumImage } = payload || {}
  ...
  } else if (action === 'fulfill' && id != null) {
    const idx = next.findIndex((r) => r.id === id)
    if (idx !== -1) {
      next[idx].status = status || 'fulfilled'
      if (fulfilledBy !== undefined) next[idx].fulfilledBy = fulfilledBy
      if (fulfilledByName !== undefined) next[idx].fulfilledByName = fulfilledByName
      if (fulfilledAt !== undefined) next[idx].fulfilledAt = fulfilledAt
      if (tabId !== undefined) next[idx].tabId = tabId
      if (albumImage !== undefined) next[idx].albumImage = albumImage
    }
  }
