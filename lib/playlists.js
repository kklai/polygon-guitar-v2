...
export async function getAutoPlaylists() {
  try {
    ...
    return snapshot.docs
      ...
      .filter(p => p.isActive !== false) // 預設顯示
...
export async function getManualPlaylists(limitCount = 8) {
  try {
    const q = query(
      collection(db, PLAYLISTS_COLLECTION),
      where('source', '==', 'manual')
  ...
  for (const [type, config] of Object.entries(AUTO_PLAYLIST_TYPES)) {
    try {
      ...
      if (snapshot.empty) {
        ...
        results.push({ type, action: 'updated', count: generated.songCount })
      }
    } catch (error) {
    ...
    for (let i = 0; i < artistIds.length; i += 10) {
      ...
      try {
        ...
        artistsSnapshot.docs.forEach(doc => {
          const data = doc.data()
          artistPhotos[doc.id] = data.photoURL || data.wikiPhotoURL || null
        })
      } catch (e) {
        console.log('Error fetching artist photos:', e)
      }
    }
  }
