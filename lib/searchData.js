 ...
 * Tabs: only fields needed for search (title, artist, composer, lyricist, arranger, uploaderPenName, arrangedBy for 出譜者).
 ...
 * Playlists: id, title, description (for 根據歌單名搜尋), coverImage.
 */
export async function buildSearchDataPayload() {
  ...
  // Build a reverse map from artist slug/normalizedName → Firestore doc ID
  // so tabs can store the stable doc ID rather than the mutable slug.
  ...
  // Tabs store artistId (Firestore doc ID) instead of artist name — name is resolved
  // from the artists array at read time (single source of truth).
...
const _API_CACHE_TTL = 3 * 60 * 1000
