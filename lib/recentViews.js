...
// 記錄歌曲瀏覽：存 main artistId（支援 artistIds-only 的 tab）及 artist 名作 fallback，顯示時用 artist map 解析名
export async function recordSongView(userId, song) {
  const image = song.coverImage || song.albumImage || song.thumbnail || getYouTubeThumbnail(song.youtubeUrl) || song.artistPhoto || null;
  const mainArtistId = getTabArtistId(song) || null;
  ...
  await addRecentView(userId, {
    ...
    artist: song.artist || song.artistName || null,
    image: image,
    thumbnail: image
  });
}
