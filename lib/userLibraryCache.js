...
import { doc, getDoc, setDoc, updateDoc, increment as firestoreIncrement } from '@/lib/firestore-tracked';
 ...
 * Falls back to rebuild if the cache read fails (e.g. rules not yet deployed).
 */
export async function getUserLibrary(userId) {
  ...
  return rebuildUserLibraryCache(userId);
}
...
/**
 * +1 or -1 liked count (atomic increment, no read required).
 */
export function patchCacheLikedCount(userId, delta) {
  if (!userId) return;
  updateDoc(doc(db, COLLECTION, userId), {
    likedCount: firestoreIncrement(delta),
    updatedAt: Date.now(),
  }).catch(() => {});
}
