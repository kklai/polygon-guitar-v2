#!/usr/bin/env node
/**
 * Export the entire tabs collection from Firestore to a single JSON file.
 * Uses startAfter() + limit() pagination to avoid loading everything into memory.
 *
 * Usage:
 *   node scripts/export-tabs-json.js                    # output: tabs-export.json
 *   node scripts/export-tabs-json.js --output=backup.json
 *   node scripts/export-tabs-json.js --limit=200         # page size (default 200)
 */
...
const OUTPUT = (() => {
  ...
  return a ? a.split('=')[1].trim() : 'tabs-export.json'
})()
const PAGE_SIZE = (() => {
...
async function main () {
  ...
  console.log('Exporting tabs to', outPath)
  ...
  while (true) {
    let q = db.collection('tabs').orderBy('__name__').limit(PAGE_SIZE)
    ...
    process.stdout.write(`  ${total} tabs written\r`)
