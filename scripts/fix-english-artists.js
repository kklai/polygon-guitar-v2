...
// Simple simp to trad conversion
...
async function searchEnWiki(name) {
  ...
  try {
    ...
    // If still no match, take first if it's a person/band
    const first = results[0];
    const snippet = first.snippet.toLowerCase();
    if (snippet.includes('singer') || snippet.includes('band') || 
        snippet.includes('musician') || snippet.includes('artist')) {
...
async function getDetails(title) {
  try {
    ...
    // Extract type
    let type = 'unknown';
    if (extract.match(/band|group|duo|trio|quartet/i)) {
      type = 'group';
    } else if (extract.match(/singer|musician|rapper|songwriter/i)) {
      if (extract.match(/\bshe\b|her\b|female|actress/i)) type = 'female';
      else if (extract.match(/\bhe\b|his\b|male|actor\b/i)) type = 'male';
    }
...
async function main() {
  console.log('處理純英文名歌手 (English Wikipedia)');
  console.log('====================================');
